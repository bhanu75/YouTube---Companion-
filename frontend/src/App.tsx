...
import { 
  Play, MessageSquare, Edit3, Save, Trash2, Plus, Eye, Calendar, User,
  FileText, Activity, Database, Search, Tag, Sparkles, Reply, X
} from 'lucide-react';
...
const YouTubeDashboard = () => {
  ...
  const loadVideoData = async () => {
    try {
      setLoading(true);
      const [videoData, commentsData, notesData] = await Promise.all([
        fetch(`${API_BASE_URL}/videos/${videoId}`).then(r => r.json()),
        fetch(`${API_BASE_URL}/comments/${videoId}`).then(r => r.json()),
        fetch(`${API_BASE_URL}/notes/${videoId}`).then(r => r.json())
      ]);
  ...
  const handleUpdateVideo = async () => {
    try {
      ...
      if (response.ok) {
        setVideo(prev => ({ ...prev, title: editedTitle, description: editedDescription }));
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/comments/${commentId}?videoId=${videoId}`, {
        method: 'DELETE'
      });
  ...
  const handleAddNote = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/notes/${videoId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: newNote, tags })
      });
  ...
  const handleDeleteNote = async (noteId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/notes/${noteId}?videoId=${videoId}`, {
        method: 'DELETE'
      });
  ...
  const handleSearchNotes = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/notes/${videoId}/search?${params}`);
      if (response.ok) {
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-md transition-colors ${
                activeTab === tab.id ? 'bg-red-100 text-red-700' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                          ...
                          disabled={loadingAI}
                          className="flex items-center space-x-1 px-3 py-1 text-xs bg-purple-50 text-purple-700 rounded-md hover:bg-purple-100 disabled:opacity-50"
                        >
                          <Sparkles className="h-3 w-3" />
                          <span>{loadingAI ? 'Loading...' : 'AI Suggestions'}</span>
                        </button>
                      </div>
                      <input
                        type="text"
                      ...
                      {aiSuggestions.length > 0 && (
                        <div className="mt-3 p-3 bg-purple-50 rounded-md">
                          <p className="text-sm font-medium text-gray-700 mb-2">AI Title Suggestions:</p>
                          {aiSuggestions.map((suggestion, idx) => (
                            <button
                              key={idx}
                              onClick={() => setEditedTitle(suggestion)}
                              className="block w-full text-left px-3 py-2 mb-1 text-sm bg-white rounded hover:bg-purple-100 transition-colors"
                            >
                              {suggestion}
                            </button>
                          ))}
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{new Date(comment.publishedAt).toLocaleDateString()}</p>
                        </div>
                      </div>
                          ...
                          <div key={reply.id} className="flex space-x-3">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center space-x-2">
                                ...
                                <p className="text-xs text-gray-500">{new Date(reply.publishedAt).toLocaleDateString()}</p>
                              </div>
                  ...
                  notes.map((note) => (
                    <div key={note.id} className="flex justify-between items-start p-4 bg-yellow-50 rounded-lg">
                      <div className="flex-1">
                        ...
                        {note.tags && note.tags.length > 0 && (
                          <div className="flex flex-wrap gap-2 mt-2">
                            {note.tags.map((tag, idx) => (
                              <span key={idx} className="inline-flex items-center px-2 py-1 bg-yellow-200 text-yellow-800 text-xs rounded">
                        ...
                        <p className="text-sm text-gray-500 mt-1">{new Date(note.created_at).toLocaleDateString()}</p>
                      </div>
                      <button
